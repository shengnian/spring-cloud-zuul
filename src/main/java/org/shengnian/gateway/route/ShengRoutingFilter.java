 ...
 * 2018/2/25
 */
public class ShengRoutingFilter extends ZuulFilter {
    ...
    public void onPropertyChange(EnvironmentChangeEvent event) {
        ...
        if (createNewClient) {
            try {
                ShengRoutingFilter.this.httpClient.close();
            }
            catch (IOException ex) {
                log.error("error closing client", ex);
            }
            ShengRoutingFilter.this.httpClient = newClient();
        }
    }

    public ShengRoutingFilter(ProxyRequestHelper helper, ZuulProperties properties) {
        this.helper = helper;
    ...
    private void initialize() {
        ...
        this.connectionManagerTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                if (ShengRoutingFilter.this.connectionManager == null) {
                    return;
                }
                ShengRoutingFilter.this.connectionManager.closeExpiredConnections();
            }
        }, 30000, 5000);
    }
